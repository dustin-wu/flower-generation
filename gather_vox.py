...
def hierarchicalfloodFill(img256,_dim):
    assert _dim==256
    img256 = np.copy(img256)
    
    #compress model 64 -> 32
    dim_voxel = 128
    img128 = np.zeros([dim_voxel,dim_voxel,dim_voxel],np.uint8)
    multiplier = int(256/dim_voxel)
    for i in range(dim_voxel):
        for j in range(dim_voxel):
            for k in range(dim_voxel):
                img128[i,j,k] = np.max(img256[i*multiplier:(i+1)*multiplier,j*multiplier:(j+1)*multiplier,k*multiplier:(k+1)*multiplier])
    
    img128 = floodFill(img128, [(0,0,0),(127,0,0),(0,127,0),(127,127,0),(0,0,127),(127,0,127),(0,127,127),(127,127,127)], 128)
                ...
                for i0 in range(-1,2):
                    for j0 in range(-1,2):
                        for k0 in range(-1,2):
                            ...
                            if img128[i+i0,j+j0,k+k0]==0: occupied_flag = False
                if occupied_flag:
                    img256[i*multiplier:(i+1)*multiplier,j*multiplier:(j+1)*multiplier,k*multiplier:(k+1)*multiplier] = np.ones([2,2,2],np.uint8)
    
    out256 = floodFill(img256, [(0,0,0),(255,0,0),(0,255,0),(255,255,0),(0,0,255),(255,0,255),(0,255,255),(255,255,255)], 256)
    
    print('filled', np.sum(np.abs(out256-img256)))
    return out256
...
def get_points_from_vox(q, name_list):
    ...
    for idx in range(name_num):
        ...
        voxel_input = "/content/FlowersBinvox/FlowersBinvox/"
...
if __name__ == '__main__':
    #dir of voxel models
    voxel_input = "FlowersBinvox/"

    #name of output file
    hdf5_path = 'flowers_vox256.hdf5'

    #obj_list
    fout = open('flowers_vox256.txt','w',newline='')
