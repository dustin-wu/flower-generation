...
# Convert a z-vector into a 3D voxel model using the trained autoencoder
def z2voxel(z, autoencoder):
    ...
    cell_coords_mat = np.zeros([dimf,dimf,dimf,dimc,dimc,dimc,3],np.float32)
    ...
    for i in range(dimf):
        for j in range(dimf):
            for k in range(dimf):
                cell_coords_mat[i,j,k,:,:,:,0] = cell_x+i*dimc
                cell_coords_mat[i,j,k,:,:,:,1] = cell_y+j*dimc
                cell_coords_mat[i,j,k,:,:,:,2] = cell_z+k*dimc
                ...
                frame_y[i,j,k] = j
                frame_z[i,j,k] = k
    cell_coords_mat = (cell_coords_mat.astype(np.float32)+0.5)/real_size-0.5
    cell_coords_mat = np.reshape(cell_coords_mat,[dimf,dimf,dimf,dimc*dimc*dimc,3])
    ...
    for i in range(frame_batch_num):
        ...
        _, model_out = autoencoder(None, z, point_coord, is_training=False)
        ...
        frame_flag[x_coords+1,y_coords+1,z_coords+1] = np.reshape((model_out>sampling_threshold), [test_point_batch_size])
    ...
    while len(queue)>0:
        ...
        for i in range(batch_num):
            ...
            cell_coords.append(cell_coords_mat[point[0]-1,point[1]-1,point[2]-1])
        ...
        model_out_batch = model_out_batch[0]
        for i in range(batch_num):
            point = point_list[i]
...
# Turns the vertices and triangles from a voxel model into a .ply file
